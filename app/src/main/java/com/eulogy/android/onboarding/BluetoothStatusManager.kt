...
class BluetoothStatusManager(
    ...
    private fun setupBluetoothEnableLauncher() {
        bluetoothEnableLauncher = activity.registerForActivityResult(
            ...
            if (isEnabled) {
                ...
                onBluetoothDisabled("Bluetooth is required for Eulogy to discover and connect to nearby users. Please enable Bluetooth to continue.")
            }
        }
    }
    ...
    fun getStatusMessage(status: BluetoothStatus): String {
        return when (status) {
            ...
            BluetoothStatus.DISABLED -> "Bluetooth is disabled. Please enable Bluetooth to use Eulogy."
