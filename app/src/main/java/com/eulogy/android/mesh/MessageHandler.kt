...
import com.eulogy.android.model.IdentityAnnouncement
import com.eulogy.android.model.RoutedPacket
import com.eulogy.android.protocol.BitchatPacket
import com.eulogy.android.protocol.MessageType
import com.eulogy.android.util.toHexString
...
class MessageHandler(private val myPeerID: String, private val appContext: android.content.Context) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket) {
        ...
        try {
            ...
            val noisePayload = com.eulogy.android.model.NoisePayload.decode(decryptedData)
            ...
            when (noisePayload.type) {
                com.eulogy.android.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                    
                    val privateMessage = com.eulogy.android.model.PrivateMessagePacket.decode(noisePayload.data)
                ...
                com.eulogy.android.model.NoisePayloadType.FILE_TRANSFER -> {
                    // Handle encrypted file transfer; generate unique message ID
                    val file = com.eulogy.android.model.BitchatFilePacket.decode(noisePayload.data)
                    if (file != null) {
                        Log.d(TAG, "🔓 Decrypted encrypted file from $peerID: name='${file.fileName}', size=${file.fileSize}, mime='${file.mimeType}'")
                        val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                        val savedPath = com.eulogy.android.features.file.FileUtils.saveIncomingFile(appContext, file)
                        val message = BitchatMessage(
                            id = uniqueMsgId,
                            sender = delegate?.getPeerNickname(peerID) ?: "Unknown",
                            content = savedPath,
                            type = com.eulogy.android.features.file.FileUtils.messageTypeForMime(file.mimeType),
                            timestamp = java.util.Date(packet.timestamp.toLong()),
                            isRelay = false,
                            isPrivate = true,
                            recipientNickname = delegate?.getMyNickname(),
                ...
                com.eulogy.android.model.NoisePayloadType.DELIVERED -> {
                ...
                com.eulogy.android.model.NoisePayloadType.READ_RECEIPT -> {
    ...
    private suspend fun sendDeliveryAck(messageID: String, senderPeerID: String) {
        try {
            ...
            val ackPayload = com.eulogy.android.model.NoisePayload(
                type = com.eulogy.android.model.NoisePayloadType.DELIVERED,
            ...
            if (encryptedPayload == null) {
                ...
                val packet = BitchatPacket(
                    ...
                    signature = null,
                    ttl = com.eulogy.android.util.AppConstants.MESSAGE_TTL_HOPS 
                )
    ...
    suspend fun handleAnnounce(routed: RoutedPacket): Boolean {
        ...
        if (age > com.eulogy.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS) {
            Log.w(TAG, "Ignoring stale ANNOUNCE from ${peerID.take(8)} (age=${age}ms > ${com.eulogy.android.util.AppConstants.Mesh.STALE_PEER_TIMEOUT_MS}ms)")
            return false
        }
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            ...
            val isFileTransfer = com.eulogy.android.protocol.MessageType.fromValue(packet.type) == com.eulogy.android.protocol.MessageType.FILE_TRANSFER
            val file = com.eulogy.android.model.BitchatFilePacket.decode(packet.payload)
            if (file != null) {
                if (isFileTransfer) {
            ...
            // Enforce verification for text messages only (not files)
            val peerInfo = delegate?.getPeerInfo(peerID)
            if (peerInfo == null || !peerInfo.isVerifiedNickname) {
                Log.w(TAG, "🚫 Dropping text message from unverified or unknown peer ${peerID.take(8)}...")
                return
            }

            // Fallback: plain text (only from verified peers)
            val message = BitchatMessage(
                sender = delegate?.getPeerNickname(peerID) ?: "unknown",
    ...
    fun restart() {
        handlerScope.cancel()
        handlerScope = CoroutineScope(Dispatchers.IO + SupervisorJob())
    }
    ...
    private fun handleFavoriteNotificationFromMesh(content: String, fromPeerID: String) {
        try {
            ...
            if (noiseKey != null) {
                com.eulogy.android.favorites.FavoritesPersistenceService.shared.updatePeerFavoritedUs(noiseKey, isFavorite)
                if (npub != null) {
                    // Index by noise key and current mesh peerID for fast Nostr routing
                    com.eulogy.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKey(noiseKey, npub)
                    com.eulogy.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(fromPeerID, npub)
                }
                ...
                val sys = com.eulogy.android.model.BitchatMessage(
                    sender = "system",
