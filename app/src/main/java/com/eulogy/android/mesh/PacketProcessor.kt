...
class PacketProcessor(private val myPeerID: String) {
    ...
    private var packetRelayManager = PacketRelayManager(myPeerID)
    
    // Coroutines
    private var processorScope = CoroutineScope(Dispatchers.IO + SupervisorJob())
...
//    }
    ...
    fun restart() {
        processorScope.cancel()
        processorScope = CoroutineScope(Dispatchers.IO + SupervisorJob())
