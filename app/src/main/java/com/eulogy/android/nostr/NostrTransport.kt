...
import com.eulogy.android.model.NoisePayloadType
...
class NostrTransport(
    ...
    companion object {
        ...
        private const val READ_ACK_INTERVAL = com.eulogy.android.util.AppConstants.Nostr.READ_ACK_INTERVAL_MS 
        ...
        transportScope.launch {
            try {
                ...
                val recipientPeerIDForEmbed = try {
                    com.eulogy.android.favorites.FavoritesPersistenceService.shared
    ...
    fun sendPrivateMessageGeohash(
        ...
        val geohash = sourceGeohash ?: run {
            val selected = try {
                com.eulogy.android.geohash.LocationChannelManager.getInstance(context).selectedChannel.value
            } catch (_: Exception) { null }
            if (selected !is com.eulogy.android.geohash.ChannelID.Location) {
    ...
    private fun resolveNostrPublicKey(peerID: String): String? {
        try {
            ...
            com.eulogy.android.favorites.FavoritesPersistenceService.shared.findNostrPubkeyForPeerID(peerID)?.let { return it }

            // 2) Legacy path: resolve by noise public key association
            val noiseKey = hexStringToByteArray(peerID)
            val favoriteStatus = com.eulogy.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKey)
            ...
            if (peerID.length == 16) {
                val fallbackStatus = com.eulogy.android.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)
