package com.eulogy.android.nostr
...
import com.eulogy.android.services.SeenMessageStore
import com.eulogy.android.ui.ChatState
import com.eulogy.android.ui.MeshDelegateHandler
import com.eulogy.android.ui.PrivateChatManager
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.eulogy.android.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (packet.type != com.eulogy.android.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch

                val noisePayload = com.eulogy.android.model.NoisePayload.decode(packet.payload) ?: return@launch
                val messageTimestamp = Date(giftWrap.createdAt * 1000L)
                val convKey = "nostr_${senderPubkey.take(16)}"
                repo.putNostrKeyMapping(convKey, senderPubkey)
                com.eulogy.android.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
    ...
    private suspend fun processNoisePayload(
        payload: com.eulogy.android.model.NoisePayload,
        convKey: String,
        ...
        when (payload.type) {
            com.eulogy.android.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                val pm = com.eulogy.android.model.PrivateMessagePacket.decode(payload.data) ?: return
            ...
            com.eulogy.android.model.NoisePayloadType.FILE_TRANSFER -> {
                ...
                val file = com.eulogy.android.model.BitchatFilePacket.decode(payload.data)
                if (file != null) {
                    val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                    val savedPath = com.eulogy.android.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = BitchatMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
