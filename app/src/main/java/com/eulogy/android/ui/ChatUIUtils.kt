...
fun formatMessageAsAnnotatedString(
    ...
    if (message.sender != "system") {
        // Get base color for this peer (pastel dark theme)
        val baseColor = if (isSelf) {
            Color(0xFFE0BBE4) // Pastel lavender-pink for self (signature color)
        } else {
        ...
        // Message content with pastel dark theme hashtag and mention highlighting
        appendIOSFormattedContent(builder, message.content, message.mentions, currentUserNickname, baseColor, isSelf, isDark)
        
        // Pastel timestamp at the END (smaller, gray-lavender)
        // Timestamp (and optional PoW badge)
        builder.pushStyle(SpanStyle(
            color = Color(0xFFA1A1AA).copy(alpha = 0.7f), // Pastel gray-lavender @70%
        ...
        // System message - pastel dark theme
        builder.pushStyle(SpanStyle(
            color = Color(0xFFB8B8C7), // Pastel gray-lavender tone
        ...
        builder.pushStyle(SpanStyle(
            color = Color(0xFFB8B8C7).copy(alpha = 0.5f), // Subtle gray-lavender
            fontSize = (BASE_FONT_SIZE - 4).sp
        ))
        builder.append(" [${timeFormatter.format(message.timestamp)}]")
        builder.pop()
...
fun formatMessageHeaderAnnotatedString(
    ...
    if (message.sender != "system") {
        val baseColor = if (isSelf) Color(0xFFE0BBE4) else getPeerColor(message, isDark)
        val (baseName, suffix) = splitSuffix(message.sender)
...
/**
 * Pastel dark theme peer color assignment using djb2 hash
 * Avoids lavender (~260-300°) reserved for self messages
 */
...
/**
 * Generate consistent pastel peer color using djb2 hash
 * Pastel dark theme: soft muted tones (HSV 35%, 90%)
 */
fun colorForPeerSeed(seed: String, isDark: Boolean): Color {
    ...
    // Avoid lavender range (~260-300°) reserved for self (pastel lavender-pink #E0BBE4)
    val lavenderStart = 260.0 / 360.0
    val lavenderEnd = 300.0 / 360.0
    if (hue >= lavenderStart && hue <= lavenderEnd) {
        // Shift out of lavender range
        hue = (hue + 0.15) % 1.0
    }
    
    // Pastel dark theme: lower saturation, high brightness for soft muted tones
    val saturation = 0.35 // Soft pastel saturation
    val brightness = 0.90 // High brightness for pastel effect
...
private fun appendIOSFormattedContent(
    ...
    for ((range, type) in allMatches) {
        ...
        when (type) {
            "mention" -> {
                // Pastel mention with hashtag suffix support
                ...
                val mentionColor = if (isMentionToMe) Color(0xFFE0BBE4) else baseColor // Pastel lavender for self-mentions
