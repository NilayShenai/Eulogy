        ...
        if (geohashPeople.isEmpty()) {
            ...
            val myHex = remember(selectedLocationChannel) {
                when (val channel = selectedLocationChannel) {
                    is com.eulogy.android.geohash.ChannelID.Location -> {
                        try {
                            val identity = com.eulogy.android.nostr.NostrIdentityBridge.deriveIdentity(
            ...
            val baseNameCounts = remember(geohashPeople) {
                ...
                geohashPeople.forEach { person ->
                    val (b, _) = com.eulogy.android.ui.splitSuffix(person.displayName)
            ...
            orderedPeople.forEach { person ->
                GeohashPersonItem(
                    ...
                    viewModel = viewModel,
                    showHashSuffix = (baseNameCounts[com.eulogy.android.ui.splitSuffix(person.displayName).first] ?: 0) > 1,
                    onTap = {
                        if (person.id != myHex) {
    ...
    Row(
        ...
        if (hasUnreadDM) {
            ...
            val (iconName, iconColor) = when {
                isMe && isMyTeleported -> "face.dashed" to Color(0xFFFFD6A5) // Soft peach for teleported me
                isTeleported -> "face.dashed" to colorScheme.onSurface // Regular color for teleported others
                isMe -> "face.smiling" to Color(0xFFFFD6A5) // Soft peach for me
        ...
        val (baseNameRaw, suffixRaw) = com.eulogy.android.ui.splitSuffix(person.displayName)
