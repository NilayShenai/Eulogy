...
fun SidebarOverlay(
    ...
    val dismissThreshold = 100f // Swipe 100dp to the right to dismiss
    ...
    Box(
        ...
        Row(
            modifier = Modifier
                ...
                .graphicsLayer {
                    translationX = offsetX.coerceAtLeast(0f) // Only allow rightward movement
                }
                .pointerInput(Unit) {
...
private fun SidebarHeader(
    ...
    // Compute channel-aware people count and color
        ...
        Row(
            ...
            if (joinedChannels.isNotEmpty()) {
                Text(
                    ...
                    style = MaterialTheme.typography.bodyMedium,
                    fontFamily = AppMonospaceFont,
                    color = if (isConnected) Color(0xFFB5EAD7) else Color(0xFFF28B82),
                    fontSize = 16.sp,
                    fontWeight = FontWeight.Medium
                )
            }
            ...
            Row(
                ...
                IconButton(
                    ...
                    Icon(
                        ...
                        contentDescription = stringResource(com.eulogy.android.R.string.cd_leave_channel),
...
fun PeopleSection(
    ...
    Column(modifier = modifier) {
        ...
        sortedPeers.forEach { pid ->
            ...
            val (b, _) = com.eulogy.android.ui.splitSuffix(dn)
            if (b != "You") baseNameCounts[b] = (baseNameCounts[b] ?: 0) + 1
        }

        // Offline favorites (exclude ones mapped to connected)
        val offlineFavorites = com.eulogy.android.favorites.FavoritesPersistenceService.shared.getOurFavorites()
        ...
        sortedPeers.forEach { peerID ->
            ...
            val (bName, _) = com.eulogy.android.ui.splitSuffix(displayName)
        ...
        offlineFavorites.forEach { fav ->
            ...
            val nostrConvKey: String? = try {
                val npubOrHex = com.eulogy.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(fav.peerNoisePublicKey)
                if (npubOrHex != null) {
                    val hex = if (npubOrHex.startsWith("npub")) {
                        val (hrp, data) = com.eulogy.android.nostr.Bech32.decode(npubOrHex)
            ...
            val dn = peerNicknames[favPeerID] ?: fav.peerNickname
            val (bName, _) = com.eulogy.android.ui.splitSuffix(dn)
            val showHash = (baseNameCounts[bName] ?: 0) > 1
...
private fun PeerItem(
    ...
    val (baseNameRaw, suffixRaw) = com.eulogy.android.ui.splitSuffix(displayName)
    ...
    val baseColor = if (isMe) Color(0xFFFBC4AB) else assignedColor // Soft peach for my messages
    ...
    Row(
        ...
        if (hasUnreadDM) {
            ...
            Icon(
                ...
                contentDescription = stringResource(com.eulogy.android.R.string.cd_unread_message),
            ...
            if (showNostrGlobe) {
                ...
                Icon(
                    ...
                    contentDescription = stringResource(com.eulogy.android.R.string.cd_reachable_via_nostr),
                    modifier = Modifier.size(16.dp),
                ...
                Icon(
                    ...
                    contentDescription = stringResource(com.eulogy.android.R.string.cd_offline_favorite),
                    modifier = Modifier.size(16.dp),
                    tint = Color.Gray
                )
            } else {
        ...
        IconButton(
            ...
            Icon(
                ...
                modifier = Modifier.size(16.dp),
                tint = if (isFavorite) Color(0xFFFDE68A) else Color(0xFFBFFCC6) // Soft yellow or mint
            )
        }
    }
}
...
private fun UnreadBadge(
    ...
    if (count > 0) {
        Box(
            modifier = modifier
                .background(
                    color = Color(0xFFFDE68A), // Soft buttery yellow
