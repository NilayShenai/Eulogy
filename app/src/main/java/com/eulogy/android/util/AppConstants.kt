package com.eulogy.android.util
...
object AppConstants {
    ...
    object Mesh {
        ...
        object Gatt {
            // Eulogy Network UUIDs (default)
            val EULOGY_SERVICE_UUID: UUID = UUID.fromString("E0719D5F-6B84-4A62-BF93-2C4D5719F40A")
            val EULOGY_CHARACTERISTIC_UUID: UUID = UUID.fromString("A1B2C3D4-E5F6-4A5B-8C9D-0E1F2A3B4C5D")
            
            
            val BITCHAT_SERVICE_UUID: UUID = UUID.fromString("F47B5E2D-4A9E-4C5A-9B3F-8E1D2C3A4B5C")
            val BITCHAT_CHARACTERISTIC_UUID: UUID = UUID.fromString("A1B2C3D4-E5F6-4A5B-8C9D-0E1F2A3B4C5D")
            
            // Common descriptor UUID (standard Bluetooth descriptor)
            val DESCRIPTOR_UUID: UUID = UUID.fromString("00002902-0000-1000-8000-00805f9b34fb")
            
            // Default service UUID (Eulogy)
            val SERVICE_UUID: UUID get() = EULOGY_SERVICE_UUID
            val CHARACTERISTIC_UUID: UUID get() = EULOGY_CHARACTERISTIC_UUID
        }
    }
    ...
    /**
     * Get the service UUID for the currently selected network
     */
    fun getServiceUUID(context: Context): UUID {
        return when (NetworkPreferenceManager.get(context)) {
            NetworkMode.EULOGY -> Mesh.Gatt.EULOGY_SERVICE_UUID
            NetworkMode.BITCHAT -> Mesh.Gatt.BITCHAT_SERVICE_UUID
        }
    }
    
    /**
     * Get the characteristic UUID for the currently selected network
     */
    fun getCharacteristicUUID(context: Context): UUID {
        return when (NetworkPreferenceManager.get(context)) {
            NetworkMode.EULOGY -> Mesh.Gatt.EULOGY_CHARACTERISTIC_UUID
            NetworkMode.BITCHAT -> Mesh.Gatt.BITCHAT_CHARACTERISTIC_UUID
        }
    }
}
